...
const ShaderCanvas = () => {
  const material = useRef(new CustomShaderMaterial()).current;
  ...
  useEffect(() => {
    ...
    const update = (time) => {
      ...
      material.uniforms.u_time.value = elapsedTime / 1000; // Convert to seconds

      animate.current = requestAnimationFrame(update);
    };
